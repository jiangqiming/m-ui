...
const buildOption = (): echarts.EChartsOption => {
  ...
  // 转换系列数据格式
  const echartsSeries: BarSeriesOption[] = seriesData.map((item: ChartBarSeries, index: number) => {
  ...
  // 计算滚动相关的数据
  const dataZoomConfig = shouldScroll.value && totalCount.value > 0
    ? {
        type: 'slider' as const,
        show: false, // 不显示滚动条
        start: Math.min((currentStartIndex.value / totalCount.value) * 100, 100),
        end: Math.min(((currentStartIndex.value + props.visibleCount) / totalCount.value) * 100, 100),
...
// 监听数据变化
watch(
  () => [props.series, props.categories, props.direction, props.colors, props.showLegend, props.barGap, props.barCategoryGap],
  () => {
    updateChart()
    // 数据变化时重新初始化滚动
    if (shouldScroll.value) {
